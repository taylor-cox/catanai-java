...
class Epsilon_Greedy_Exploration(Base_Exploration_Strategy):
    ...
    def calculate_epsilon_with_cyclical_strategy(self, episode_number):
        ...
        max_epsilon = 0.5
        min_epsilon = 0.001
        increment = (max_epsilon - min_epsilon) / float(self.exploration_cycle_episodes_length / 2) # type: ignore
        cycle = [ix for ix in range(int(self.exploration_cycle_episodes_length / 2))] + [ix for ix in range( # type: ignore
            int(self.exploration_cycle_episodes_length / 2), 0, -1)] # type: ignore

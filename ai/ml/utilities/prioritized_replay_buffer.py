from ai.ml.ddqn_her.max_heap import Max_Heap
from ai.ml.ddqn_her.deque import Deque
from ai.ml.ddqn_her.node import Node
import numpy as np
import torch
...
class Prioritised_Replay_Buffer(Max_Heap, Deque):
    ...
    def swap_heap_elements(self, index1, index2):
        ...
        self.heap[index1].heap_index = index1  # type: ignore
        self.heap[index2].heap_index = index2  # type: ignore
    ...
    def update_td_errors(self, td_errors):
        ...
        for raw_td_error, deque_index in zip(td_errors, self.deque_sample_indexes_to_update_td_error_for):
            td_error =  (abs(raw_td_error) + self.incremental_td_error) ** self.alpha
            corresponding_heap_index = self.deque[deque_index].heap_index  # type: ignore
            self.update_overall_sum(td_error, self.heap[corresponding_heap_index].key)  # type: ignore
            self.heap[corresponding_heap_index].key = td_error  # type: ignore

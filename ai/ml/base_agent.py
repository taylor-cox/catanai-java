...
class Base_Agent(object):
    ...
    def reset_game(self):
        ...
        if "exploration_strategy" in self.__dict__.keys(): self.exploration_strategy.reset() #type: ignore
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        if self.config.save_model: self.locally_save_policy()  #type: ignore
    ...
    def show_whether_achieved_goal(self):
        ...
        if index_achieved_goal == -1: #this means agent never achieved goal
            print("\033[91m" + "\033[1m" +
                  "{} did not achieve required score \n".format(self.agent_name) +  #type: ignore
                  "\033[0m" + "\033[0m")
        else:
            print("\033[92m" + "\033[1m" +
                  "{} achieved required score at episode {} \n".format(self.agent_name, index_achieved_goal) +  #type: ignore
                  "\033[0m" + "\033[0m")
    ...
    def update_learning_rate(self, starting_lr,  optimizer):
        ...
        if len(self.rolling_results) > 0:
            ...
            for g in optimizer.param_groups:
                g['lr'] = new_lr
        if random.random() < 0.001: self.logger.info("Learning rate {}".format(new_lr))  #type: ignore

    def enough_experiences_to_learn_from(self):
        """Boolean indicated whether there are enough experiences in the memory buffer to learn from"""
        return len(self.memory) > self.hyperparameters["batch_size"]  #type: ignore

    def save_experience(self, memory=None, experience=None):
        """Saves the recent experience to the memory buffer"""
        if memory is None: memory = self.memory  #type: ignore
    ...
    def take_optimisation_step(self, optimizer, network, loss, clipping_norm=None, retain_graph=False):
        ...
        if clipping_norm is not None:
            for net in network:
                torch.nn.utils.clip_grad_norm_(net.parameters(), clipping_norm) #clip gradients to help stabilise training  #type: ignore
    ...
    def log_gradient_and_weight_information(self, network, optimizer):
        ...
        for g in optimizer.param_groups:
            learning_rate = g['lr']
            break
        self.logger.info("Learning Rate {}".format(learning_rate))  #type: ignore

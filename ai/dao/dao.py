...
class GameStateDAO:
    ...
    def addGamestate(self, gamestate: str, game_id: int, numActionsUntilSuccessful: int, reward: float, action: List[int], agent: str = ''):
        self.recreate_session()
        ...
        gamestateToAdd = GameState(
            ...
            robberIndex=gamestateDict['robberIndex'][0][0],
            agent=agent,
            actionStateID=gamestateDict['actionState'][0][0]
        )
    ...
    def getLargestGameID(self) -> int:
        self.recreate_session()
        largestGamesState = self.s.query(GameState).order_by(
            GameState.game_id.desc()).first()
        largestGameID: int | None = None
        if largestGamesState is not None:
            largestGameID = largestGamesState.game_id  # type: ignore
        self.s.close()
        return largestGameID if largestGameID is not None else 0
